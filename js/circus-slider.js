    ...
    options = $.extend(
      {
        ...
        /* Determines if the buttons for left or right scrolling should be
         * displayed
         */
        'showButtons':
          false,
        ...
        /* If this option and 'showThumbnails' is both true, this option will
         * use the first appering image in the li element as a representing
         * thumbnail for that slide.
         */
        'useImagesForThumbnails':
          false,

        /* Allows the user to specify a wrapper that containes the thumbnail
         * container
        ...
        /* Creating the ability to hook in to the slider before the animation
         * has begun.
         */
        'preSlide':

          /* @param tick int How far we will slide, negative number for
           * backwards and positive for forward.
           * @param currentIndex int The index of the fronted image before
           * sliding.
           * @param nextIndex int The index of the fronted image after slidig
           */
        ...
        'postSlide':
          ...
          /* @param tick int How far we have slided, negative number for
           * backwards and positive for forward.
           * @param formerIndex int The index of the fronted image before
           * slidig.
           * @param currentIndex int The index of the fronted image after
           * slidig.
           */
          function( tick, formerIndex, currentIndex ){},

        /* Creating the ability to hook a listener to the thumbnail. If
         * stopPropagation is called upon the event then the sliding will be
         * prevented.
         */
        'thumbnailClick':

          /* @param thumbnail Element The thumnail element that was clicked upon
           * @param event Event The click event generated by clicking the
           * thumbnail
           */
          function( thumbnail, event ){},

        /* Creating the ability to hook a listener to the left button. If
         * stopPropagation is called upon the event then the sliding will be
         * prevented.
         */
        'leftButtonClick':
           ...
           * @param event Event The click event generated by clicking the button
           */
          function( button, event ){},

        /* Creating the ability to hook a listener to the right button. If
         * stopPropagation is called upon the event then the sliding will be
         * prevented.
         */
        'rightButtonClick':
    ...
    $( this )
      ...
      .each(
        function()
        {
          ...
          /* Setting an index class to the li elements to better be able to
           * keep track of them.
           */
          ...
          /* How many li elements exists in the initiation part. This tells us
           * how many images acctuly existes after cloning has accured.
           */
          ...
          /* Cloning elements
           *
           * We need 5 li element for a nice flow and a good looking floded
           * view.
           */
          ...
          var viewPort = $( '<div />' ).addClass( 'circus-slider-viewport' );
          ...
          /* Setting positions so we start from the beginning but still have
           * two element behind us.
           */
          ...
          /* A flag that determines if an animation is currently running.
           */
          ...
          slide = function( tick )
          {
            // Prevents this animation from starting if there's already an
            // animation running
            if( ani )
            {
              // Cashed event will run after current animation has finished
              eventCash = tick;
            ...
            ul.css(
              {
                ...
                // Setting new size to be able to display cloned elements
                'width':
                  ( ul.width() + length ) + 'px'
              });
                ...
                ul.css(
                  {
                    ...
                    // Setting new size after the dublicates has been removed
                    'width':
                      ( ul.width() - length ) + 'px'
                  });
          ...
          if( options.showButtons )
          {
            ...
            leftButton = $( '<div />' )
              ...
              /* Appending an empty element on the button that could be used
          ...
          if( options.showThumbnails )
          {
            ...
            for( var i = 0, l = iniLiLength; i < l; i++ )
              thumbnailContainer.
                append(
                  $( '<div />' )
                    .addClass( 'circus-slider-thumbnail' )
                    ...
                    .click(
                      function( e )
                      {
                        ...
                        options.thumbnailClick( this, e );

                        // If the hook stopped the propagation than that will
                        // prevent sliding
                        if( e.isPropagationStopped() )
                          return;

                        // Calculating a relevent path
                        var go = $( this ).attr( 'data-thumbnail' ) - viewIndex;
            ...
            $( '> .circus-slider-thumbnail:first-child', thumbnailContainer )
            ...
            /* Setting images as thumbnails if desired by the user, using first
             * appering image
             */
            ...
            if( options.useImagesForThumbnails )
            {
              ...
              $( '> div', thumbnailContainer ).each(
                function()
                {
                  // The first image in the sliding position will be
                  // represented as the thumbnail
                  var img = $( 'img:first', li[ i++ ] );

                  // Will not set any thumbnail if no image is found
                  if( img.length == 1 )
                  {
                    // Creating a new object so that we have a relative scope
                    // for every thumbnail.
                    new ( function( _this, src, w, h )
                    {
                      var img = new Image();

                      // Doing the magic onece the image is done loading
                      img.onload = function()
                      {
                        // Setting the thumbnail sizes with some offset value
                        var
                        width  = w * 1.2,
                        height = h * 1.2;
                        ...
                        // Adding the thumbnail to its container
                        $( _this ).append( img );
                      };

                      // Setting the image source after we sat the onload event
                      // handler
                      img.src = src;
          ...
          if( typeof options.delay == 'number' )
          {
            ...
            intervalID = setInterval( intervalCallback, options.delay );
